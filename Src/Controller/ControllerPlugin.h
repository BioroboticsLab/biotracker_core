...
/**
 * This is the controller class of the Plugin Loader Component. This component
 * is responsible for loading and managing BioTracker Plugins The
 * ControllerPlugin class is responsible for loading and connecting to
 * BioTracker Plugins. It also manages tracking data modification commands. It
 * queues up the commands and forwards them to the trakcing plugin if either
 * the tracking is paused or the tracker is done with tracking the previous
 * frame. The plgin then sends the commands first and afterward the next image
 * to track. This avoids a race condition.
 */
...
public:
    ...
    /**
     * (This function
     * This function delivers a file path of a Plugin to the IModel class
     * PluginLoader. If the Plugin could be loaded, the functions additionally
     * requests the name of the Plugin and sets it as an entry into an IView
...
signals:
    ...
    void emitMoveElement(IModelTrackedTrajectory* element,
                         uint                     frameNumber,
                         QPoint                   pos);
    ...
    void emitEntityRotation(IModelTrackedTrajectory* trajectory0,
                            double                   angle,
                            uint                     frameNumber);
...
private Q_SLOTS:
    /**
     *
     * If Tracking is active and the tracking process was finished, the Plugin
     * is able to emit a Signal that triggers this SLOT.
     */
    ...
    /**
     *
     * Receive command to move a element in a trajectory and put it in edit
     * queue
     */
    ...
    /**
     *
     * Receive command to fix a traj, so the tracker should not change it's
     * data
     */
