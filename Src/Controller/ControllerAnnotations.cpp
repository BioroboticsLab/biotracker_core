...
ControllerAnnotations::ControllerAnnotations(QObject*              parent,
...
{
    {
        ...
        QObject::connect(view,
                         ...
                         this,
                         &ControllerAnnotations::mousePressEvent,
                         Qt::DirectConnection);
        QObject::connect(view,
                         &GraphicsView::onMouseReleaseEvent,
                         this,
                         &ControllerAnnotations::mouseReleaseEvent,
                         Qt::DirectConnection);
        QObject::connect(view,
                         &GraphicsView::onMouseMoveEvent,
                         this,
                         &ControllerAnnotations::mouseMoveEvent,
                         Qt::DirectConnection);
        // QObject::connect(view, &GraphicsView::onKeyReleaseEvent, this,
        // &ControllerAnnotations::keyReleaseEvent, Qt::DirectConnection);
        QObject::connect(view,
                         &GraphicsView::onKeyPressEvent,
                         this,
                         &ControllerAnnotations::keyPressEvent,
                         Qt::DirectConnection);

        QWidget* viewport = view->viewport();
        QObject::connect(this,
                         SIGNAL(onRepaintRequired()),
                         viewport,
                         SLOT(repaint()));
    }

    {
        ...
        MediaPlayer* player = dynamic_cast<MediaPlayer*>(
            mediaPlayerController->getModel());
        assert(player);
        QObject::connect(player,
                         &MediaPlayer::fwdPlayerParameters,
                         this,
                         &ControllerAnnotations::setPlayerParameters);
    }
}
...
void ControllerAnnotations::mousePressEvent(QMouseEvent*  event,
                                            const QPoint& pos)
{
...
Annotations::TrackedPoint ControllerAnnotations::snapToTrajectory(
    const QPoint& originalPoint)
{
    ...
    if (trackedTrajectoryModel &&
        QGuiApplication::queryKeyboardModifiers().testFlag(
            Qt::ControlModifier)) {
        ...
        for (const auto& childNode : allChildren) {
            ...
            const auto childTrajectory =
                dynamic_cast<IModelTrackedTrajectory*>(childNode);
            ...
            for (size_t i = 0; i < childTrajectory->size(); ++i) {
                if (i != model->getCurrentFrame())
                    continue;
                const auto& childComponent = childTrajectory->getChild(
                    static_cast<int>(i));
                ...
                float distance = std::sqrt(
                    std::pow(point->getXpx() - originalPoint.x(), 2) +
                    std::pow(point->getYpx() - originalPoint.y(), 2));
                if (distance < minDistance) {
                    minDistance    = distance;
                    closestPoint   = QPoint(static_cast<int>(point->getXpx()),
...
{
    auto       model = static_cast<Annotations*>(getModel());
    const bool isAltModifierActive =
        QGuiApplication::queryKeyboardModifiers().testFlag(Qt::AltModifier);

    Annotations::TrackedPoint trackedPoint = snapToTrajectory(pos);

    if ((event->button() == Qt::LeftButton) && !isAltModifierActive &&
        (model->endAnnotation(trackedPoint) ||
         model->updateAnnotation(trackedPoint))) {
        updateView();
        event->accept();
    }
}

void ControllerAnnotations::mouseMoveEvent(QMouseEvent*  event,
                                           const QPoint& pos)
{
    auto       model = static_cast<Annotations*>(getModel());
    const bool isAltModifierActive =
    ...
    if ((event->buttons() & Qt::LeftButton) && !isAltModifierActive &&
        model->updateAnnotation(trackedPoint)) {
        updateView();
        event->accept();
    }
    ...
    std::shared_ptr<const playerParameters> parameters)
{
    ...
    if (trackedTrajectoryModel != nullptr)
        model->updateTrackedAnnotations(
            trackedTrajectoryModel->getChildNodes());
    updateView();
}
