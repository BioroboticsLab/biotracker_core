...
void ControllerPlugin::addToPluginList(QString str)
{
    ...
    QPointer<ControllerMainWindow> ctrMainWindow =
        qobject_cast<ControllerMainWindow*>(ctrA);
...
void ControllerPlugin::loadPluginFromFileName(QString str)
{
    ...
    if (pluginLoader->loadPluginFromFilename(str)) {
        ...
        IController* ctrB = m_BioTrackerContext->requestController(
            ENUMS::CONTROLLERTYPE::TRACKEDCOMPONENTCORE);
        QPointer<ControllerTrackedComponentCore> ctrTrackedComponentCore =
            qobject_cast<ControllerTrackedComponentCore*>(ctrB);

        // Add Tracker tracked components model to
        // ControllerTrackedComponentCore
        ctrTrackedComponentCore->addModel(
            m_BioTrackerPlugin->getTrackerComponentModel());
        // Add tracked component view to main window
        ctrMainWindow->setCoreElementsWidget(
            ctrTrackedComponentCore->getTrackingElementsWidgetCore());

        IController* ctrDE = m_BioTrackerContext->requestController(
            ENUMS::CONTROLLERTYPE::DATAEXPORT);
        QPointer<ControllerDataExporter> ctrDataExp =
            qobject_cast<ControllerDataExporter*>(ctrDE);
        ctrDataExp->setDataStructure(
            m_BioTrackerPlugin->getTrackerComponentModel());
        ctrDataExp->setComponentFactory(
            m_BioTrackerPlugin->getComponentFactory());
...
void ControllerPlugin::loadPluginsFromPluginSubfolder()
{
    ...
    std::vector<std::string> list = _cfg->UseRegistryLocations == 1
                                        ? PluginLoader::queryRegistryBehaviors(
                                              REGISTRY_PATH)
                                        : std::vector<std::string>();
    // Search directories
    std::vector<std::string> filesFromFolders =
        PluginLoader::searchDirectoriesForPlugins(list, TRACKER_SUFFIX);
...
void ControllerPlugin::connectControllerToController()
{
    ...
    QObject::connect(this,
                     SIGNAL(emitUpdateView()),
                     ctrTrackedComponentCore,
                     SLOT(receiveUpdateView()));

    // connect ControllerCommands
    IController* ctrD = m_BioTrackerContext->requestController(
        ENUMS::CONTROLLERTYPE::COMMANDS);
    QPointer<ControllerCommands> ctrCommands =
        qobject_cast<ControllerCommands*>(ctrD);

    QObject::connect(ctrCommands,
                     SIGNAL(emitAddTrajectory(QPoint)),
                     this,
    ...
    QObject::connect(ctrCommands,
                     ...
                     this,
                     SLOT(receiveRemoveTrajectoryId(int)),
                     Qt::DirectConnection);
    QObject::connect(
        ctrCommands,
        SIGNAL(emitRemoveTrackEntity(IModelTrackedTrajectory*, uint)),
        this,
        SLOT(receiveRemoveTrackEntity(IModelTrackedTrajectory*, uint)),
        Qt::DirectConnection);
    QObject::connect(ctrCommands,
                     SIGNAL(emitValidateTrajectory(int)),
                     this,
                     SLOT(receiveValidateTrajectory(int)),
                     Qt::DirectConnection);
    QObject::connect(
        ctrCommands,
        SIGNAL(emitValidateEntity(IModelTrackedTrajectory*, uint)),
        this,
        SLOT(receiveValidateEntity(IModelTrackedTrajectory*, uint)),
        Qt::DirectConnection);
    QObject::connect(
        ctrCommands,
        SIGNAL(emitMoveElement(IModelTrackedTrajectory*, uint, QPoint, int)),
        this,
        SLOT(receiveMoveElement(IModelTrackedTrajectory*, uint, QPoint, int)),
        Qt::DirectConnection);
    QObject::connect(ctrCommands,
                     SIGNAL(emitSwapIds(IModelTrackedTrajectory*,
                                        IModelTrackedTrajectory*)),
                     this,
                     SLOT(receiveSwapIds(IModelTrackedTrajectory*,
                                         IModelTrackedTrajectory*)),
                     Qt::DirectConnection);
    QObject::connect(
        ctrCommands,
        ...
        this,
        SLOT(receiveToggleFixTrack(IModelTrackedTrajectory*, bool)),
        Qt::DirectConnection);
    QObject::connect(
        ctrCommands,
        SIGNAL(emitEntityRotation(IModelTrackedTrajectory*, double, uint)),
        this,
        SLOT(receiveEntityRotation(IModelTrackedTrajectory*, double, uint)),
        Qt::DirectConnection);

    // connect ControllerPlayer
    IController* ctrC = m_BioTrackerContext->requestController(
        ENUMS::CONTROLLERTYPE::PLAYER);
    QPointer<ControllerPlayer> ctrPlayer = qobject_cast<ControllerPlayer*>(
        ctrC);
    QObject::connect(ctrPlayer,
                     SIGNAL(emitPauseState(bool)),
                     this,
                     SLOT(receivePauseState(bool)),
                     Qt::DirectConnection);

    QObject::connect(ctrPlayer,
                     SIGNAL(signalCurrentFrameNumberToPlugin(uint)),
                     this,
                     SLOT(receiveCurrentFrameNumberToPlugin(uint)),
                     Qt::DirectConnection);
}
...
void ControllerPlugin::createPlugin()
{
    ...
    ControllerAreaDescriptor* ctAreaDesc =
        qobject_cast<ControllerAreaDescriptor*>(ctrAreaDesc);
...
void ControllerPlugin::connectPlugin()
{
    ...
    ControllerTextureObject* ctrTexture =
        qobject_cast<ControllerTextureObject*>(ctrB);
    ...
    IController* ctrData = m_BioTrackerContext->requestController(
        ENUMS::CONTROLLERTYPE::DATAEXPORT);
    ControllerDataExporter* ctDataEx = qobject_cast<ControllerDataExporter*>(
        ctrData);

    IController* ctrC = m_BioTrackerContext->requestController(
        ENUMS::CONTROLLERTYPE::TRACKEDCOMPONENTCORE);
    ControllerTrackedComponentCore* ctrCompView =
        qobject_cast<ControllerTrackedComponentCore*>(ctrC);

    IController* ctrD = m_BioTrackerContext->requestController(
        ENUMS::CONTROLLERTYPE::COREPARAMETER);
    ControllerCoreParameter* ctrCoreParam =
        qobject_cast<ControllerCoreParameter*>(ctrD);

    IController* ctrE = m_BioTrackerContext->requestController(
        ENUMS::CONTROLLERTYPE::MAINWINDOW);
    QPointer<ControllerMainWindow> ctrMainWindow =
    ...
    QObject::connect(ctAreaDesc,
                     ...
                     obj,
                     SLOT(receiveAreaDescriptor(IModelAreaDescriptor*)));

    QObject::connect(
        obj,
        SIGNAL(emitCorePermission(std::pair<ENUMS::COREPERMISSIONS, bool>)),
        ctrCompView,
    ...
    connect(this,
            &ControllerPlugin::frameRetrieved,
            m_BioTrackerPlugin,
            &IBioTrackerPlugin::receiveCurrentFrameFromMainApp);
    QObject::connect(this,
                     SIGNAL(signalCurrentFrameNumberToPlugin(uint)),
                     obj,
    ...
    if (m_BioTrackerPlugin) {
        while (!m_editQueue.isEmpty()) {
            ...
            case EDIT::ROTATE_ENTITY:
                emitEntityRotation(edit.trajectory0,
                                   edit.angle,
                                   edit.frameNumber);
                break;
            }
...
void ControllerPlugin::receiveRemoveTrajectory(
...
void ControllerPlugin::receiveRemoveTrackEntity(
    IModelTrackedTrajectory* trajectory,
    uint                     frameNumber)
{
    if (m_paused) {
...
void ControllerPlugin::receiveMoveElement(IModelTrackedTrajectory* trajectory,
                                          uint                     frameNumber,
                                          QPoint                   pos,
                                          int                      toMove)
{
...
void ControllerPlugin::receiveSwapIds(IModelTrackedTrajectory* trajectory0,
...
void ControllerPlugin::receiveValidateEntity(
    IModelTrackedTrajectory* trajectory,
    uint                     frameNumber)
{
    if (m_paused) {
...
void ControllerPlugin::receiveToggleFixTrack(
    IModelTrackedTrajectory* trajectory,
    bool                     toggle)
{
    if (m_paused) {
...
void ControllerPlugin::receiveEntityRotation(
    IModelTrackedTrajectory* trajectory,
    double                   angle,
    uint                     frameNumber)
{
