...
ControllerPlayer::ControllerPlayer(QObject*              parent,
...
void ControllerPlayer::loadVideoStream(
                                             ...
                                             uint                     number)
{
    ...
    QPointer<ControllerPlugin> ctrPlugin = qobject_cast<ControllerPlugin*>(
        ctr);
...
int ControllerPlayer::recordOutput()
{
    ...
    QPointer<ControllerGraphicScene> ctrTextureObject =
        qobject_cast<ControllerGraphicScene*>(ctr);

    return qobject_cast<MediaPlayer*>(m_Model)->toggleRecordGraphicsScenes(
        dynamic_cast<GraphicsView*>(ctrTextureObject->getView()));
}
...
QString ControllerPlayer::takeScreenshot()
{
    ...
    QPointer<ControllerGraphicScene> ctrTextureObject =
        qobject_cast<ControllerGraphicScene*>(ctr);
    return qobject_cast<MediaPlayer*>(m_Model)->takeScreenshot(
        dynamic_cast<GraphicsView*>(ctrTextureObject->getView()));
}
...
void ControllerPlayer::connectControllerToController()
{
    ...
    // IController* ictrCpv =
    // m_BioTrackerContext->requestController(ENUMS::CONTROLLERTYPE::COREPARAMETER);
    // QPointer< ControllerCoreParameter > ctrCpv =
    // dynamic_cast<ControllerCoreParameter*>(ictrCpv);

    // QObject::connect(this, &ControllerPlayer::emitPauseState, ctrCpv,
...
void ControllerPlayer::createView()
{
    IController* ctr = m_BioTrackerContext->requestController(
        ENUMS::CONTROLLERTYPE::MAINWINDOW);
    QPointer<MainWindow> mainWindow = dynamic_cast<MainWindow*>(
        ctr->getView());
...
void ControllerPlayer::connectModelToController()
{
    ...
    QObject::connect(qobject_cast<MediaPlayer*>(m_Model),
                     &MediaPlayer::renderCurrentImage,
                     this,
                     &ControllerPlayer::receiveRenderImage);
    QObject::connect(qobject_cast<MediaPlayer*>(m_Model),
                     &MediaPlayer::trackCurrentImage,
                     this,
                     &ControllerPlayer::receiveImageToTracker);
    QObject::connect(this,
                     &ControllerPlayer::emitPauseState,
                     qobject_cast<MediaPlayer*>(m_Model),
                     &MediaPlayer::rcvPauseState);
    QObject::connect(qobject_cast<MediaPlayer*>(m_Model),
                     &MediaPlayer::signalVisualizeCurrentModel,
                     this,
                     &ControllerPlayer::receiveVisualizeCurrentModel);
    QObject::connect(qobject_cast<MediaPlayer*>(m_Model),
                     &MediaPlayer::signalCurrentFrameNumberToPlugin,
                     this,
                     &ControllerPlayer::receiveCurrentFrameNumberToPlugin);

    QObject::connect(qobject_cast<MediaPlayer*>(m_Model),
                     &MediaPlayer::emitNextMediaInBatch,
                     this,
                     &ControllerPlayer::receiveNextMediaInBatch,
                     Qt::DirectConnection);
    QObject::connect(qobject_cast<MediaPlayer*>(m_Model),
                     &MediaPlayer::emitNextMediaInBatchLoaded,
                     this,
                     &ControllerPlayer::receiveNextMediaInBatchLoaded,
                     Qt::DirectConnection);

    IController* ctrTRCC = m_BioTrackerContext->requestController(
        ENUMS::CONTROLLERTYPE::TRACKEDCOMPONENTCORE);
    QPointer<ControllerTrackedComponentCore> trCC =
        qobject_cast<ControllerTrackedComponentCore*>(ctrTRCC);

    QObject::connect(trCC,
                     &ControllerTrackedComponentCore::emitTrackNumber,
                     this,
                     &ControllerPlayer::receiveTrackCount,
                     Qt::DirectConnection);
}
...
void ControllerPlayer::receiveVisualizeCurrentModel(uint frameNumber)
{
    IController* ctr = m_BioTrackerContext->requestController(
        ENUMS::CONTROLLERTYPE::TRACKEDCOMPONENTCORE);
    QPointer<ControllerTrackedComponentCore> ctrTrCompCore =
