...
/**
    ...
    Annotations will be drawn on top of the video. Their position is defined in
   image coordinates.
*/
class Annotations : public IModel
{
    Q_OBJECT
public:
    Annotations(std::string filepath = "")
    : filepath(filepath)
    {
        deserialize();
    }
    ...
    struct TrackedPoint
    {
        TrackedPoint(const QPoint& pos)
        : position(pos)
        {
        }
        TrackedPoint(int x, int y)
        : TrackedPoint(QPoint(x, y))
        {
        }
        TrackedPoint(const QPoint& pos, int trackID)
        : TrackedPoint(pos)
        {
        ...
        // Update the current position to the corresponding track position IIF
        // isLinkedToTrack.
        void update(int                                   currentFrameID,
    ...
    */
    struct Annotation
    {
        Annotation(QPoint origin = {0, 0}, size_t startFrame = 0)
        : origin(origin)
        ...
        QString text{""};
        /// Name that identifies this type of annotation and is used for
        /// serialization.
        ...
        virtual bool onEndAnnotation(TrackedPoint currentPosition)
        {
        ...
        {
            return QRect(*origin, *origin).marginsAdded({20, 20, 20, 20});
        }
        ...
        {
            if (isHandleAtPosition(origin, pos))
                return &origin;
            return nullptr;
        }
        // Static, so that the view can use it to draw special handles.
        static void drawHandleLocation(QPainter* painter,
                                       QPoint    pos,
                                       QString   text);
        static void drawHandleLocation(QPainter*           painter,
                                       const TrackedPoint& pos,
                                       QString             text)
        {
            drawHandleLocation(painter, *pos, text);
        }
        virtual void updateTrackedPositions(
            int                                   currentFrameID,
            const QList<IModelTrackedComponent*>& trackedComponents)
        {
    ...
    struct AnnotationArrow : public Annotation
    {
        ...
        virtual bool onEndAnnotation(TrackedPoint currentPosition) override
        {
            arrowHead = currentPosition;
            return true;
        }
        virtual void   paint(QPainter*                       painter,
                             const QStyleOptionGraphicsItem* option,
        ...
        virtual QRectF boundingRect() const
        {
            return QRect(*origin, *arrowHead).marginsAdded({20, 20, 20, 20});
        }
        virtual TrackedPoint* getHandleForPosition(const QPoint& pos) override;
        virtual void          updateTrackedPositions(
                     int                                   currentFrameID,
                     const QList<IModelTrackedComponent*>& trackedComponents) override;
    ...
    // Called by the controller during mouse-drags to update the current
    // annotation.
...
private:
    ...
    std::string getFilename() const; /**< The resulting filename will add a
                                        suffix to the original source path. */

    mutable bool dirty{
        false}; /**< Whether the annotations need to be serialized on exit.  */

    std::vector<std::shared_ptr<Annotation>>
        annotations; /**< vector of all annotations  */

    std::shared_ptr<Annotation>
        currentAnnotation; /**< Held temporarily during events - not yet
                              'created'.  */

    /// Valid during drag & drop or after mouse selection.
    struct SelectionData
    {
        // Annotation from the annotations vector above. Defines validity of
        // the whole struct.
        ...
        Annotation* operator->()
        {
            return annotation.lock().get();
        }
        void reset()
        {
            return annotation.reset();
        }
    } selection;
