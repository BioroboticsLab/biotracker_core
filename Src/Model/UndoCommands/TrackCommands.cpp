...
RemoveTrackCommand::RemoveTrackCommand(IModelTrackedTrajectory* traj,
                                       QUndoCommand*            parent)
: QUndoCommand(parent)
, _traj(traj)
{
...
RemoveElementCommand::RemoveElementCommand(IModelTrackedTrajectory* traj,
                                           uint          frameNumber,
                                           QUndoCommand* parent)
: QUndoCommand(parent)
, _traj(traj)
, _frameNumber(frameNumber)
{
    ...
    setText("Remove entity " + QString::number(_frameNumber) +
            " of trajectory " + QString::number(traj->getId()));
}
...
MoveElementCommand::MoveElementCommand(IModelTrackedTrajectory* traj,
                                       uint                     frameNumber,
                                       QPoint                   oldPos,
                                       QPoint                   newPos,
                                       int                      toMove,
...
, _toMove(toMove)
{
    setText("Move entity " + QString::number(_frameNumber) +
            " of trajectory " + QString::number(traj->getId()) + " to " +
            QString::number(_newPos.x()) + ", " +
            QString::number(_newPos.y()));
}
...
//{
//	const MoveElementCommand *moveCommandToMerge = static_cast<const
...
SwapTrackIdCommand::SwapTrackIdCommand(IModelTrackedTrajectory* traj0,
...
, _traj1(traj1)
{
    setText("Swap id's of " + QString::number(_traj0->getId()) + " and " +
            QString::number(_traj1->getId()));
}
...
FixTrackCommand::FixTrackCommand(IModelTrackedTrajectory* traj,
                                 bool                     isFixed,
                                 QUndoCommand*            parent)
: QUndoCommand(parent)
, _traj(traj)
...
{
    if (_isFixed) {
        setText("Fix tracking data of trajectory " +
                QString::number(_traj->getId()));
    } else {
        setText("Unfix tracking data of trajectory " +
                QString::number(_traj->getId()));
    }
}
...
RotateEntityCommand::RotateEntityCommand(IModelTrackedTrajectory* traj,
                                         double                   oldAngle,
                                         double                   angle,
                                         uint                     frameNumber,
                                         QUndoCommand*            parent)
...
, _frameNumber(frameNumber)
{
    setText("Rotate entity " + QString::number(_frameNumber) +
            " of trajectory " + QString::number(_traj->getId()) + " to " +
            QString::number(_newAngle));
}
