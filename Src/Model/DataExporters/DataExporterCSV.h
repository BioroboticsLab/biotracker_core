...
private:
     ...
     *
     * Similar getHeaderElements, but for all components and having all
     * elements of the return vector concatenated returns
     * GlobalHeaders,Individual1,Indiviual2,...
     */
    std::string getHeader(IModelTrackedComponent* comp, int cnt);

    /* Sets sets a property to a components.
     */
    void setProperty(IModelTrackedComponent* comp,
                     std::string             key,
                     std::string             val);
    ...
    void addChildOfChild(IModelTrackedTrajectory*       root,
