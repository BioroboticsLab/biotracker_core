...
std::string DataExporterSerialize::writeTrackpoint(IModelTrackedPoint* e,
                                                   int trajNumber)
{
    std::stringstream ss;
...
void DataExporterSerialize::loadFile(std::string file)
{
    ...
    for (int i = 0; i < children; i++) {
        IModelTrackedTrajectory* child = static_cast<IModelTrackedTrajectory*>(
    ...
    for (int i = 0; i < childids.size(); i++) {
        IModelTrackedTrajectory* curTraj =
            static_cast<IModelTrackedTrajectory*>(root->getChild(childids[i]));
        int trajSize = -1;
        in >> trajSize;
        ...
        for (int idx = 0; idx < t->size(); idx++) {
            if (t) {
                IModelTrackedComponent* e =
                    dynamic_cast<IModelTrackedComponent*>(t->getChild(idx));
                if (e) {
                    cnt++;
                }
            }
