...
void Annotations::serialize() const
{
    ...
    for (auto& annotation : annotations) {
        serializedAnnotations.append(
            QJsonObject::fromVariantMap(annotation->serializeToMap()));
    }
...
void Annotations::deserialize()
{
    ...
    const QJsonDocument jsonDocument{
        QJsonDocument::fromJson(infile.readAll())};
...
void Annotations::updateTrackedAnnotations(
    const QList<IModelTrackedComponent*>& trackedComponents)
{
    for (auto& annotation : annotations) {
        annotation->updateTrackedPositions(static_cast<int>(getCurrentFrame()),
                                           trackedComponents);
    }
}

void Annotations::TrackedPoint::update(
    int                                   currentFrameID,
    const QList<IModelTrackedComponent*>& trackedComponents)
{
    if (!isLinkedToTrack)
    ...
    for (const auto& trackedComponent : trackedComponents) {
        const auto trackedTrajectory = dynamic_cast<IModelTrackedTrajectory*>(
            trackedComponent);
        if (trackedTrajectory == nullptr)
            continue;
        if (trackedTrajectory->getId() != trackID)
            continue;

        const auto& childComponent = trackedTrajectory->getChild(
            currentFrameID);
        const auto point = dynamic_cast<IModelComponentEuclidian2D*>(
            childComponent);
        if (point == nullptr)
            return;

        position = QPoint(static_cast<int>(point->getXpx()),
                          static_cast<int>(point->getYpx()));
        return;
    }
}

void Annotations::Annotation::drawHandleLocation(QPainter* painter,
                                                 QPoint    pos,
                                                 QString   text)
{
    ...
    // so text can be read when annotation is also contrasting with the
    // background
    QPen   textPen{original};
    QColor textColor{dotted.color()};
    textColor.setHsv(textColor.hsvHue() + 20,
                     textColor.hsvSaturation(),
                     textColor.value());
...
bool Annotations::Annotation::isHandleAtPosition(const QPoint& handle,
                                                 const QPoint& pos)
{
    QPoint      diff      = handle - pos;
    const float euclidian = std::sqrt((diff.x() * diff.x()) +
                                      (diff.y() * diff.y()));
    return euclidian <= 20.0;
}

void Annotations::Annotation::deserializeFrom(
    const QMap<QString, QVariant>& map)
{
    text = map.value("comment", text).toString();
    startFrame =
        map.value("start_frame", static_cast<int>(startFrame)).toInt();
...
void Annotations::AnnotationLabel::paint(
    QPainter*                       painter,
    ...
    QWidget*                        widget) const
{
    ...
    painter->setPen(thin);
    const int len = 20;
    painter->drawLine(*this->origin + QPoint(-len, -len),
                      *this->origin + QPoint(+len, +len));
    painter->drawLine(*this->origin + QPoint(-len, len),
                      *this->origin + QPoint(+len, -len));
...
void Annotations::AnnotationArrow::deserializeFrom(
    const QMap<QString, QVariant>& map)
{
...
void Annotations::AnnotationArrow::paint(
    QPainter*                       painter,
    const QStyleOptionGraphicsItem* option,
    QWidget*                        widget) const
{
...
Annotations::TrackedPoint* Annotations::AnnotationArrow::getHandleForPosition(
    const QPoint& pos)
{
    if (isHandleAtPosition(arrowHead, pos))
        return &arrowHead;
    return Annotation::getHandleForPosition(pos);
}

void Annotations::AnnotationArrow::updateTrackedPositions(
    int                                   currentFrameID,
    const QList<IModelTrackedComponent*>& trackedComponents)
{
...
void Annotations::AnnotationRect::paint(QPainter* painter,
                                        const QStyleOptionGraphicsItem* option,
                                        QWidget* widget) const
{
...
{
    currentAnnotation = std::make_shared<AnnotationArrow>(origin,
                                                          currentFrame);
}

void Annotations::startLabel(QPoint origin, size_t currentFrame)
{
    currentAnnotation = std::make_shared<AnnotationLabel>(origin,
                                                          currentFrame);
}
...
{
    currentAnnotation = std::make_shared<AnnotationEllipse>(origin,
                                                            currentFrame);
}
...
bool Annotations::trySetText(QPoint cursor)
{
    ...
    for (auto& annotation : annotations) {
        ...
        bool    ok;
        QString annoText = QInputDialog::getText(Q_NULLPTR,
                                                 tr("Set annotation text"),
                                                 tr("Annotation text:"),
                                                 QLineEdit::Normal,
                                                 annotation->getText(),
                                                 &ok);
