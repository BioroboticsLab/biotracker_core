...
/**
 * The MediaPlayer class is an IModel class an part of the MediaPlayer
 * component. This class creats a MediaPlayerStateMachine object and moves it
 * to a QThread. The MediaPlayer class controlls and represents the results of
 * the MediaPlayerStateMachine. The communication between a MediaPlayer object
 * and a MediaPlayerStateMachine object is done by Qts SIGNALS and SLOTS. This
 * is due to the fact that SIGNALS and SLTOS are thread safe. A disadvantage is
 * that SIGNALS and SLOTS are slower then normal function calls.
 *
 * The constructor of this class is responsible for connecting to the
...
Q_SIGNALS:
    /**
     * Emit the path to a video stream. This signal will be received by the
     * MediaPlayerStateMachine which runns in a separate Thread.
     */
    void loadVideoStream(std::vector<boost::filesystem::path> files);
    /**
     * Emit the path to pictures. This signal will be received by the
     * MediaPlayerStateMachine which runns in a separate Thread.
     */
    void loadPictures(std::vector<boost::filesystem::path> files);
    /**
     * Emit the camera device number. This signal will be received by the
     * MediaPlayerStateMachine which runns in a separate Thread.
     */
    void loadCameraDevice(CameraConfiguration conf);

    /**
     * Emit a frame number. This signal will be received by the
     * MediaPlayerStateMachine which runns in a separate Thread.
     */
    void goToFrame(int frame);
    /**
     * Emit the next frame command. This signal will be received by the
     * MediaPlayerStateMachine which runns in a separate Thread.
     */
    void nextFrameCommand();
    /**
     * Emit previous frame command. This signal will be received by the
     * MediaPlayerStateMachine which runns in a separate Thread.
     */
    void prevFrameCommand();
    /**
     * Emit the play command. This signal will be received by the
     * MediaPlayerStateMachine which runns in a separate Thread.
     */
    void playCommand();
    /**
     * Emit stop command. This signal will be received by the
     * MediaPlayerStateMachine which runns in a separate Thread.
     */
    void stopCommand();
    /**
     * Emit the pause command. This signal will be received by the
     * MediaPlayerStateMachine which runns in a separate Thread.
     */
    void pauseCommand();

    /**
     * This SIGNAL will be emmited if a state operation should be executed.
     * This signal will be received by the MediaPlayerStateMachine which runns
     * in a separate Thread.
     */
    void runPlayerOperation();

    /**
     * This SIGNAL will send a cv::Mat and a name to the MediaPlayer controller
...
public Q_SLOTS:
    /**
     * MediaPlayer will receive the current playerParameters from the
     * MediaPlayerStateMachine.
     */
    void receivePlayerParameters(
        std::shared_ptr<const playerParameters> param);

    /**
     * If the MediaPlayerStateMachine is finished with executing the current
     * state it will trigger this SLOT.
     */
     ...
     * We do this although the info is basically passed implicitly via other
     * commands, because it's easier
     */
...
private:
    ...
    /**
     * helper function which opens a video. If video size has changed, a new
     * video is opened.
     */
