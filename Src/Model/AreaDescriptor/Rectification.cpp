...
Rectification::Rectification(float w, float h)
{
    initRecitification(w, h);
    setupRecitification(0, 0, 0, 0);
}

void Rectification::init(double                 areaHeight_cm,
                         double                 areaWidth_cm,
                         std::vector<cv::Point> areaCoordinates,
                         int                    camCaptureWidth_px,
                         int                    camCaptureHeight_px,
                         int                    frameDisplayWidthPx,
                         int                    frameDisplayHeightPx)
{
    ...
    setupRecitification(frameDisplayWidthPx,
                        frameDisplayHeightPx,
                        camCaptureWidth_px,
                        camCaptureHeight_px);
}

void Rectification::initRecitification(double areaHeight_cm,
                                       double areaWidth_cm)
{
    setArea();
...
void Rectification::setArea(std::vector<cv::Point> areaCoordinates)
{
    ...
    // BioTracker::Util::TypedSingleton<BioTracker::Core::Settings>::getInstance(CORE_CONFIGURATION);
    //_settings->setParam(AREADESCRIPTOR::CN_ARENA,
    // cvPointsToString(areaCoordinates).c_str());
}
...
void Rectification::setArea(std::vector<QPoint> areaCoordinates)
{
    ...
    for (int i = 0; i < areaCoordinates.size(); i++) {
        cvAreaCoordinates.push_back(
            cv::Point(areaCoordinates[i].x(), areaCoordinates[i].y()));
    }
...
{
    std::string s = (Config::DefaultArena).toStdString();
    return stringToCVPointVec(s);
}
...
void Rectification::setDimension(double areaWidth_cm, double areaHeight_cm)
{
    _areaWidth_cm  = areaWidth_cm;
    _areaHeight_cm = areaHeight_cm;
    setupRecitification(_frameDisplayWidthPx,
                        _frameDisplayHeightPx,
                        _camCaptureWidth_px,
                        _camCaptureHeight_px);
}

void Rectification::setupRecitification(int frameDisplayWidthPx,
                                        int frameDisplayHeightPx,
                                        int camImageWidth,
                                        int camImageHeight)
{
    ...
    _camCaptureWidth_px  = frameDisplayWidthPx;
    _camCaptureHeight_px = frameDisplayHeightPx;
    ...
    _rectifiedAreaCoordinates.push_back(
        cv::Point2f(0.0, _areaHeight_cm)); // botton left
    _rectifiedAreaCoordinates.push_back(
        cv::Point2f(_areaWidth_cm, _areaHeight_cm)); // bottom right
    _rectifiedAreaCoordinates.push_back(
        cv::Point2f(_areaWidth_cm, 0.0)); // top right
...
void Rectification::resetAreaCoordinates()
{
    ...
    double rationHeight = double(_frameDisplayHeightPx) /
                          double(_camCaptureHeight_px);
    double rationWidth = double(_frameDisplayWidthPx) /
                         double(_camCaptureWidth_px);
    ...
    if (checkValidCoordinates(areaCoordinates,
                              _camCaptureHeight_px,
                              _camCaptureWidth_px)) {
...
bool Rectification::checkValidCoordinates(
    std::vector<cv::Point> areaCoordinates,
    int                    camCaptureHeight_px,
    int                    camCaptureWidth_px)
{
    for (cv::Point p : areaCoordinates) {
        if (p.y > camCaptureHeight_px)
            return false;
        if (p.x > camCaptureWidth_px)
            return false;
    }
...
{
    bool flag = (point_cm.x >= 0 && point_cm.y >= 0 &&
                 point_cm.x <= _areaWidth_cm && point_cm.y <= _areaHeight_cm);
    return flag;
}
...
cv::Point Rectification::getCorrectedResolutionPoint(cv::Point pos)
{
    ...
    if (_camCaptureWidth_px > _frameDisplayWidthPx ||
        _camCaptureHeight_px > _frameDisplayHeightPx) {
        ...
        if (_camCaptureWidth_px == _camCaptureHeight_px) {
            if (heightScaled > _frameDisplayHeightPx) {
                scaleFactor = float(_camCaptureHeight_px) /
                              float(_frameDisplayHeightPx);
    ...
    return cv::Point(int(float(pos.x) * scaleFactor),
                     int(float(pos.y) * scaleFactor));
}