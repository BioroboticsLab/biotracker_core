...
{
    _parms = nullptr;
    _areaInfoCache = static_cast<IControllerCfg*>(parent)->getConfig()->AreaDefinitions;
...
void AreaInfo::reset(int w, int h) {
    ...
    QVector<QString> vertices = _parms != nullptr ? getVertices(_parms->m_CurrentFilename, _areaInfoCache) : QVector<QString>();
...
void AreaInfo::loadAreas() {
    ...
    QVector<QString> pair = getVertices(_parms ? _parms->m_CurrentFilename : "", _areaInfoCache, true);
...
void AreaInfo::rcvPlayerParameters(playerParameters* parameters)
{
    ...
    if ((m->size().width != _vdimX || m->size().height != _vdimY) &&
        _useEntireScreen) { //TODO: _useEntireScreen? if we do not update stuff in the very beginning, tracking rectification will break
        reset(m->size().width, m->size().height);
        loadAreas();
        updateRectification();
    }
...
void AreaInfo::updateRectification() {
    if (_parms) {
        if (!_rectInitialized) {
            QVector<QString> vertices = getVertices(_parms->m_CurrentFilename, _areaInfoCache);
            std::vector<QPoint> pts = toQPointVector(vertices[0]);
