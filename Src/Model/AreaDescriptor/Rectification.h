...
public:
    ...
    {
        static Rectification _instance(100,
                                       100); // Guaranteed to be destroyed.
...
private:
    ...
    Rectification(
        const Rectification&); /* verhindert, dass eine weitere Instanz via
                                  Kopier-Konstruktor erstellt werden kann */

    Rectification& operator=(
        const Rectification&); // Verhindert weitere Instanz durch Kopie

public:
    void init(double                 areaHeight_cm,
              double                 areaWidth_cm,
    ...
    double areaWidth_cm() const
    {
        return _areaWidth_cm;
    }
    double areaHeight_cm() const
    {
        return _areaHeight_cm;
    }
    int captureWidth_px() const
    {
        return _camCaptureWidth_px;
    }
    int captureHeight_px() const
    {
    ...
    void setCamImageSize(cv::Size size)
    {
        _camCaptureWidth_px  = size.width;
        _camCaptureHeight_px = size.height;
    }

    void initRecitification(double areaHeight_cm, double areaWidth_cm);
    void setupRecitification(int frameDisplayWidthPx,
                             int frameDisplayHeightPx,
                             int camImageWidth,
                             int camImageHeight);

    std::vector<cv::Point> area() const
    {
        return _areaCoordinates;
    }
...
private:
    ...
    bool checkValidCoordinates(std::vector<cv::Point> areaCoordinates,
                               int                    camCaptureHeight_px,
                               int                    camCaptureWidth_px);
};
