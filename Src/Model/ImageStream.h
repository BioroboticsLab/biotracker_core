...
namespace BioTracker
{
    namespace Core
    {
        ...
        /**
         * The ImageStream class was part of BioTracker version 2. It is
         * responsible for generation ImageStreams from files or camera
         * devices.
         */
        ...
        public:
            ...
            /**
             * @return true, if the stream's end is reached; i.e. current frame
             * points behind it's last frame
             */
            bool end() const;
            ...
            /**
             * @return the filename of the current file defined through the
             * frameNumber
             */
             ...
             * @return true if the operation was successful, i.e. the frame
             * number is valid and no error occurred.
             */
            bool setFrameNumber(size_t frame_number);

            /**
             * advances the current frame frame.
             * - if this function is called on the media's last frame, the
             * current frame is invalidated.
             * @return true if the operation was successful, i.e. the current
             * frame isn't the last frame and no error occurred.
             */
            bool nextFrame();

            /**
             * sets the current frame position to the previous frame.
             * - if this function is called on the media's first frame, the
             * current frame is invalidated.
             * @return true if the operation was successful, i.e. the current
             * frame isn't the first one and no error occurred.
             */
            bool previousFrame();

            /**
             * Gets the title of the current image stream.
             * A title should represent the identity of a source stream as a
             *string. E.g. a video "MyTracking.mkv" should yield a name like
             *"MyTracking". Streams from Camera No. 1 should yiel "Camera_1" or
             *the like. Naming conventions are up to the stream
             *implementations.
             */
            std::string getTitle();
        ...
        protected:
             ...
             *"MyTracking". Streams from Camera No. 1 should yiel "Camera_1" or
             *the like. Naming conventions are up to the stream
             *implementations. The title is set by the stream itself and may
             *not be modified from the outside.
             */
            void setTitle(std::string title);

            /**
             * The stride of the image stream. Think of it as "use only every
             * n'th frame".
             */
        ...
        private:
            /**
             * empties m_current_frame & sets m_current_frame_number to
             * this->numFrames();
             */
            void clearImage();
            /**
             * - called by ImageStreamImpl::setFrameNumber
             *    if frame_number < numFrames() && frame_number !=
        ...
        std::shared_ptr<ImageStream> make_ImageStream3Video(
            Config*                                     cfg,
            const std::vector<boost::filesystem::path>& filename);
