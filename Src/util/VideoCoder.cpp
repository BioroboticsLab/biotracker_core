                           ...
                           bool                         dropFrames)
{
    ...
    //" MB). Exiting. " << std::endl; 	std::exit(1);
    //}
    _Access.unlock();
}
...
void Worker::run()
{
    ...
    if (m_nvEncoder) {
        ...
        while (1) {
            ...
            cv::Mat writeMat;
            cv::cvtColor(*(mat->_img),
                         writeMat,
                         CV_BGR2YUV_I420); // CV_BGR2YUV_I420 //CV_BGR2YUV
...
int VideoCoder::toggle(int w, int h, double fps)
{
    ...
    if (!m_recording) {
        ...
        if (codecStr == "X264") {
            ...
            vWriter   = std::make_shared<cv::VideoWriter>(
                getTimeAndDate(videoDir + "CameraCapture", ".avi"),
                codec,
                fps,
                cv::Size(w, h),
                1);
            ...
            cfg->inputFormat =
                NV_ENC_BUFFER_FORMAT_NV12; // NV_ENC_BUFFER_FORMAT_NV12;//NV_ENC_BUFFER_FORMAT_YUV444
            const std::string f = getTimeAndDate(std::string(CFG_DIR_VIDEOS) +
                                                     "CameraCapture",
                                                 ".avi");
