...
namespace BioTracker
{
    namespace Core
    {
        namespace ScreenHelper
        {
             ...
             */
            inline float calculate_viewport(const int im_w,
                                            const int im_h,
                                            const int w,
                                            const int h,
                                            QRect&    window,
                                            QRect&    viewport)
            {
                ...
                // we later need to adjust an offset caused the use of
                // different dimensions for window and viewport.
             ...
             * Calculates the actual dimension of the image with all zoom, pan
             * and viewport transformations applied
             * @return a rectangle that represents the position and dimension
             * of the image in the videoview element
             */
            inline QRect getImDimsInScreenCoords(const PanZoomState zoomState,
                                                 const int          im_w,
                                                 const int          im_h,
                                                 const int          w,
                                                 const int          h)
            {
                ...
                QRect actualIm(
                    static_cast<int>(realPanX),
                    static_cast<int>(realPanY),
             ...
             */
            inline QPoint imageToScreenCoords(const PanZoomState zoomState,
                                              const int          im_w,
                                              const int          im_h,
                                              const int          w,
                                              const int          h,
            ...
            {
                QPoint      result;
                const QRect actualIm =
                    getImDimsInScreenCoords(zoomState, im_w, im_h, w, h);
                const float one_step_x = actualIm.width() /
                                         static_cast<float>(im_w);
                const float one_step_y = actualIm.height() /
                                         static_cast<float>(im_h);
                const int actualPosXInIm = static_cast<int>(
                    round(poi.x() * one_step_x));
                const int actualPosYInIm = static_cast<int>(
                    round(poi.y() * one_step_y));
                ...
                QPoint                               poi)
            {
                ...
                const float transformedX = (-actualIm.x() + poi.x()) *
                                           one_step_x;
                const float transformedY = (-actualIm.y() + poi.y()) *
                                           one_step_y;
            ...
            /**
             * @brief zoomTo
             * Modifies the panzoomstate so that the cursor (zoomCenter) will
             * point to the same location in the image as prior to the zoom
             ...
             */
            inline PanZoomState zoomTo(PanZoomState state,
                                       const int    im_w,
                                       const int    im_h,
                                       const int    w,
                                       const int    h,
                                       ...
                                       const QPoint zoomCenter)
            {
                ...
                const QPoint imPos =
                    screenToImageCoords(state, im_w, im_h, w, h, zoomCenter);

                const float oldZoomFactor = state.zoomFactor;
                const float newZoomFactor = state.zoomFactor -
                                            (deltaZoom / 2000);
                ...
                // As we do not want to zoom to (0/0) but rather keep the focus
                // on the chosen image position (zoomCenter) we now have to
                // "back-translate" to our initial position
                const QPoint translatedZoomCenter =
                    imageToScreenCoords(state, im_w, im_h, w, h, imPos);
