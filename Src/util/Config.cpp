...
void Config::load(QString dir, QString file)
{
    ...
    config->DisableWizard       = tree.get<int>(globalPrefix + "DisableWizard",
                                          config->DisableWizard);
    config->FrameStride         = tree.get<int>(globalPrefix + "FrameStride",
                                        config->FrameStride);
    config->RectificationHeight = tree.get<double>(
        globalPrefix + "RectificationHeight",
        config->RectificationHeight);
    config->RectificationWidth = tree.get<double>(globalPrefix +
                                                      "RectificationWidth",
                                                  config->RectificationWidth);
    config->AppertureType      = tree.get<int>(globalPrefix + "AppertureType",
                                          config->AppertureType);
    config->CsvSeperator   = tree.get<QString>(globalPrefix + "CsvSeperator",
                                             config->CsvSeperator);
    config->CoreConfigFile = tree.get<QString>(globalPrefix + "CoreConfigFile",
                                               config->CoreConfigFile);
    config->VideoCodecUsed = tree.get<int>(globalPrefix + "VideoCodecUsed",
                                           config->VideoCodecUsed);
    config->DropFrames     = tree.get<int>(globalPrefix + "DropFrames",
                                       config->DropFrames);
    config->RecordScaledOutput = tree.get<int>(globalPrefix +
                                                   "RecordScaledOutput",
                                               config->RecordScaledOutput);
    config->DataExporter       = tree.get<int>(globalPrefix + "DataExporter",
                                         config->DataExporter);
    config->RecordFPS          = tree.get<int>(globalPrefix + "RecordFPS",
                                      config->RecordFPS);
    config->CameraWidth        = tree.get<int>(globalPrefix + "CameraWidth",
                                        config->CameraWidth);
    config->CameraHeight       = tree.get<int>(globalPrefix + "CameraHeight",
                                         config->CameraHeight);
    config->GpuQp = tree.get<double>(globalPrefix + "GpuQp", config->GpuQp);
    config->DefaultLocationManualSave = tree.get<QString>(
        globalPrefix + "DefaultLocationManualSave",
        config->DefaultLocationManualSave);
    config->DirPlugins     = tree.get<QString>(globalPrefix + "DirPlugins",
                                           config->DirPlugins);
    config->DirVideos      = tree.get<QString>(globalPrefix + "DirVideos",
                                          config->DirVideos);
    config->DirTracks      = tree.get<QString>(globalPrefix + "DirTracks",
                                          config->DirTracks);
    config->DirTrials      = tree.get<QString>(globalPrefix + "DirTrials",
                                          config->DirTrials);
    config->DirScreenshots = tree.get<QString>(globalPrefix + "DirScreenshots",
                                               config->DirScreenshots);
    config->DirTemp        = tree.get<QString>(globalPrefix + "DirTemp",
                                        config->DirTemp);
    config->AreaDefinitions      = tree.get<QString>(globalPrefix +
                                                    "AreaDefinitions",
                                                config->AreaDefinitions);
    config->UseRegistryLocations = tree.get<int>(globalPrefix +
                                                     "UseRegistryLocations",
                                                 config->UseRegistryLocations);
}
...
void Config::save(QString dir, QString file)
{
    ...
    tree.put(globalPrefix + "RectificationHeight",
             config->RectificationHeight);
    ...
    tree.put(globalPrefix + "DefaultLocationManualSave",
             config->DefaultLocationManualSave);
    ...
    tree.put(globalPrefix + "UseRegistryLocations",
             config->UseRegistryLocations);
