...
, ui(new Ui::MainWindow)
{
    ...
    QApplication::setWindowIcon(
        QIcon(":/Logo/resources/logo/BT-3_non_transparent_rounded.ico"));
}
...
void MainWindow::setupUpperToolBar()
{
    ...
    // leave space at the top for the title */}" "QGroupBox::title
    // {subcontrol-origin: margin;subcontrol-position: top center; padding: 0
    // 3px;}");
...
void MainWindow::addCoreParameterView(IView* coreParameterView)
{
    ...
    if (coreParameter) {
        ...
        QWidget* experimentWidget = coreParameter->findChild<QWidget*>(
            "experimentWidget");
        if (experimentWidget) {
        ...
        hLayout->setContentsMargins(
            QMargins(0, 0, 0, 0)); // left, top, right, bottom
        hLayout->setMargin(0);
        ...
        splitterSizes << 20000 << 10000; // 2:1 TODO make this fixed (right
                                         // panel fixed start size)
...
void MainWindow::createIntroductionWizard()
{
    ...
    // BioTracker::Core::Settings *disableIntroWiz =
    // GET_CORESETTINGS(CORE_CONFIGURATION);
    ...
    QObject::connect(noShowCheck,
                     &QCheckBox::toggled,
                     this,
                     &MainWindow::toggleNoShowWiz);
    ...
    QWizardPage* p1 = new QWizardPage;
    {
        ...
        QPixmap scaledImg = logoImg.scaled(QSize(600, 400),
                                           Qt::KeepAspectRatio,
        ...
        imgLabel->setSizePolicy(QSizePolicy::Expanding,
    ...
    QWizardPage* p2 = new QWizardPage;
    {
        ...
        QLabel* label = new QLabel(
            "This is the BioTracker. Don't let yourself be overwhelmed as it "
            "is actually pretty simple.");
        label->setWordWrap(true);

        QPixmap overviewImg(
            ":/Introduction/resources/introduction/images/BioTracker.PNG");
        QPixmap scaledImg = overviewImg.scaled(QSize(600, 400),
                                               Qt::KeepAspectRatio,
                                               Qt::SmoothTransformation);
    ...
    QWizardPage* p3 = new QWizardPage;
    {
        ...
        // image
        QPixmap overviewImg(
            ":/Introduction/resources/introduction/images/toolbars.png");
        QPixmap scaledImg = overviewImg.scaled(QSize(600, 400),
                                               Qt::KeepAspectRatio,
                                               Qt::SmoothTransformation);
        QLabel* imgLabel  = new QLabel;
    ...
    QWizardPage* p4 = new QWizardPage;
    {
        ...
        // image
        QPixmap overviewImg(
            ":/Introduction/resources/introduction/images/right_panel.png");
        QPixmap scaledImg = overviewImg.scaled(QSize(600, 400),
                                               Qt::KeepAspectRatio,
                                               Qt::SmoothTransformation);
        QLabel* imgLabel  = new QLabel;
    ...
    QWizardPage* p5 = new QWizardPage;
    {
        ...
        // image
        QPixmap overviewImg(
            ":/Introduction/resources/introduction/images/tracking.png");
        QPixmap scaledImg = overviewImg.scaled(QSize(600, 400),
                                               Qt::KeepAspectRatio,
                                               Qt::SmoothTransformation);
        QLabel* imgLabel  = new QLabel;
    ...
    QWizardPage* p6 = new QWizardPage;
    {
        ...
        // images
        QPixmap contextEntityImg(
            ":/Introduction/resources/introduction/images/context_entity.png");
        QPixmap scaledImg1     = contextEntityImg.scaled(QSize(300, 400),
                                                     Qt::KeepAspectRatio,
                                                     Qt::SmoothTransformation);
        QLabel* imgEntityLabel = new QLabel;
        imgEntityLabel->setPixmap(scaledImg1);

        QPixmap contextVideoImg(
            ":/Introduction/resources/introduction/images/context_video.png");
        QPixmap scaledImg2    = contextVideoImg.scaled(QSize(300, 400),
                                                    Qt::KeepAspectRatio,
                                                    Qt::SmoothTransformation);
...
void MainWindow::setCorePermission(
...
{
    QString posString = QString("%1, %2").arg(QString::number(pos.x()),
...
void MainWindow::saveDataToFile()
{
    ...
    if (f != "") {
        qobject_cast<ControllerMainWindow*>(getController())
            ->saveTrajectoryFile(f.toStdString());
    }
}
...
void MainWindow::on_rightPanelViewControllerButton_clicked()
{
    ...
    if (ui->widgetParameterAreaOuterCanvas->isVisible()) {
        _lastRightPanelWidth =
            ui->mainSplitter->sizes()[1]; // save last width to restore on show
        _lastVideoWidgetWidth = ui->mainSplitter->sizes()[0];

        ui->widgetParameterAreaOuterCanvas->hide();
        splitterSizes
            << 20000
            << ui->rightPanelViewControllerWidget->minimumSizeHint().width();
...
{
    ui->videoControls->setVisible(
        ui->bottomPanelViewControllerButton->text() == "^");
    ui->bottomPanelViewControllerButton->setText(
        ui->bottomPanelViewControllerButton->text() == "v" ? "^" : "v");
}
...
void MainWindow::on_actionOpen_Video_triggered()
{
    static const QString videoFilter(
        "Video files (*.avi *.wmv *.mp4 *.mkv *.mov)");

    QString filename =
        QFileDialog::getOpenFileName(this, "Open video", "", videoFilter, 0);

    if (!filename.isEmpty()) {
        dynamic_cast<ControllerMainWindow*>(getController())
            ->loadVideo({filename.toStdString().c_str()});
    }
}
...
{
    static const QString pluginFilter(
        "BioTracker Tracking Plugin files (*.bio_tracker.so *.bio_tracker.dll "
        "*.bio_tracker.dylib)");

    QString filename = QFileDialog::getOpenFileName(
        this,
        "Open BioTracker Tracking Plugin",
        "",
        pluginFilter,
        0);
...
void MainWindow::on_actionOpen_Picture_triggered()
{
    static const QString imageFilter(
        "image files (*.png *.jpg *.jpeg *.gif *.bmp *.jpe *.ppm *.tiff *.tif "
        "*.sr *.ras *.pbm *.pgm *.jp2 *.dib)");
...
void MainWindow::on_actionLoad_trackingdata_triggered()
{
    ...
    if (!files.empty()) {
        qobject_cast<ControllerMainWindow*>(getController())
            ->loadTrajectoryFile(files[0].string());
    }
}
...
void MainWindow::on_actionOpen_Camera_triggered()
{
    ...
    QObject::connect(m_CameraDevice,
                     &CameraDevice::emitSelectedCameraDevice,
                     this,
                     &MainWindow::receiveSelectedCameraDevice);
}
...
void MainWindow::on_actionToggle_menu_toolbar_triggered()
{
    ...
    ui->actionToggle_menu_toolbar->setText(
        !currentState ? "Hide menu toolbar" : "Show menu toolbar");
}
void MainWindow::on_actionToggle_view_toolbar_triggered()
{
    ...
    ui->actionToggle_view_toolbar->setText(
        !currentState ? "Hide view toolbar" : "Show view toolbar");
}
void MainWindow::on_actionToggle_video_toolbar_triggered()
{
    ...
    ui->actionToggle_video_toolbar->setText(
        !currentState ? "Hide view toolbar" : "Show view toolbar");
}

void MainWindow::on_actionToggle_compact_menu_toolbar_2_triggered()
{
    ui->toolBarMenu->setStyleSheet(
        "QGroupBox"
...
{
    ui->actionToggle_menu_toolbar->setText(visible ? "Hide menu toolbar"
                                                   : "Show menu toolbar");
}
void MainWindow::on_toolBarTools_visibilityChanged(bool visible)
{
    ui->actionToggle_view_toolbar->setText(visible ? "Hide view toolbar"
                                                   : "Show view toolbar");
}
void MainWindow::on_toolBarVideo_visibilityChanged(bool visible)
{
    ui->actionToggle_video_toolbar->setText(visible ? "Hide video toolbar"
                                                    : "Show video toolbar");
}
...
void MainWindow::on_actionUser_guide_triggered()
{
    QDesktopServices::openUrl(
        QUrl("https://github.com/BioroboticsLab/biotracker_core/wiki"));
}
...
void MainWindow::on_actionShortcuts_triggered()
{
    ...
    while (!shortcutFile.atEnd()) {
        ...
        while (!stream.atEnd()) {
            ...
            if (shortCutAndDescription.size() == 2) {
                scList.append(
                    QPair<QString, QString>(shortCutAndDescription[0],
                                            shortCutAndDescription[1]));
            }
        }
    }
    ...
    for (sc = scList.constBegin(); sc != scList.constEnd(); ++sc) {
        ...
        scKey->setFlags(Qt::NoItemFlags | Qt::ItemIsEnabled |
                        Qt::ItemIsSelectable);
        scKeyInfo->setFlags(Qt::NoItemFlags | Qt::ItemIsEnabled |
                            Qt::ItemIsSelectable);
