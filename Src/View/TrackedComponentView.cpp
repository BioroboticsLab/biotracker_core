...
/// set permissions for this and all component shapes, which were send by the plugin
...
void TrackedComponentView::connectShape(ComponentShape* shape) {
	CoreParameter* coreParams = dynamic_cast<CoreParameter*>
		(dynamic_cast<ControllerTrackedComponentCore*>(getController())->getCoreParameter());
	...
	QObject::connect(shape, SIGNAL(emitRemoveTrajectory(IModelTrackedTrajectory*)),
		dynamic_cast<ControllerTrackedComponentCore*>(this->getController()),
		SIGNAL(emitRemoveTrajectory(IModelTrackedTrajectory*)), Qt::DirectConnection);
	QObject::connect(shape, SIGNAL(emitRemoveTrackEntity(IModelTrackedTrajectory*, uint)),
		dynamic_cast<ControllerTrackedComponentCore*>(this->getController()),
		SIGNAL(emitRemoveTrackEntity(IModelTrackedTrajectory*, uint)), Qt::DirectConnection);
	QObject::connect(shape, SIGNAL(emitMoveElement(IModelTrackedTrajectory*, QPoint, QPoint, uint, int)),
		dynamic_cast<ControllerTrackedComponentCore*>(this->getController()),
		SIGNAL(emitMoveElement(IModelTrackedTrajectory*, QPoint, QPoint, uint, int)), Qt::DirectConnection);
	QObject::connect(shape, SIGNAL(emitToggleFixTrack(IModelTrackedTrajectory*, bool)),
		dynamic_cast<ControllerTrackedComponentCore*>(this->getController()),
		SIGNAL(emitToggleFixTrack(IModelTrackedTrajectory*, bool)), Qt::DirectConnection);
	QObject::connect(shape, SIGNAL(emitEntityRotation(IModelTrackedTrajectory*, double, double, uint)),
		dynamic_cast<ControllerTrackedComponentCore*>(this->getController()),
		SIGNAL(emitEntityRotation(IModelTrackedTrajectory*, double, double, uint)), Qt::DirectConnection);
	QObject::connect(shape, &ComponentShape::emitGoToFrame,
		dynamic_cast<ControllerTrackedComponentCore*>(this->getController()),
		&ControllerTrackedComponentCore::emitGoToFrame, Qt::DirectConnection);
	...
	shape->setPermission(std::pair<ENUMS::COREPERMISSIONS, bool>
		(ENUMS::COREPERMISSIONS::COMPONENTMOVE, m_permissions[ENUMS::COREPERMISSIONS::COMPONENTMOVE]));
	shape->setPermission(std::pair<ENUMS::COREPERMISSIONS, bool>
		(ENUMS::COREPERMISSIONS::COMPONENTREMOVE, m_permissions[ENUMS::COREPERMISSIONS::COMPONENTREMOVE]));
	shape->setPermission(std::pair<ENUMS::COREPERMISSIONS, bool>
		(ENUMS::COREPERMISSIONS::COMPONENTSWAP, m_permissions[ENUMS::COREPERMISSIONS::COMPONENTSWAP]));
	shape->setPermission(std::pair<ENUMS::COREPERMISSIONS, bool>
		(ENUMS::COREPERMISSIONS::COMPONENTROTATE, m_permissions[ENUMS::COREPERMISSIONS::COMPONENTROTATE]));
...
/**
* updates componentshapes, deletes all if tracking data empty and creates new if new trajectories were added
* componentshapes are children of this view (see qt's parent-child-system)
*/
...
* if not broadcasted, only the position of actually moved componentshape is saved in tracking data
* the rest would be reset after update
*/
	...
	foreach(entity, allSelectedEntitiesList) {
		...
		if (shape) {
			if (shape->isSelected()) {
				...
				//calculate the current pos and emit it to the commands component
				shape->emitMoveElement(shapeTrajectory, shape->getOldPos() + QPoint(shape->m_w / 2, shape->m_h / 2),
					shape->pos().toPoint() +
					QPoint(shape->m_w / 2, shape->m_h / 2), m_currentFrameNumber, numberSelectedEntitiesToMove);
...
void TrackedComponentView::receiveColorChangeBorderAll()
{
	...
	QColor color = QColorDialog::getColor(Qt::white, nullptr,
		QString("Choose border color"), QColorDialog::ShowAlphaChannel);
	foreach(childItem, childrenItems) {
		ComponentShape* childShape = dynamic_cast<ComponentShape*>(childItem);
		if (childShape) {
...
void TrackedComponentView::receiveColorChangeBrushSelected()
{
	...
	QGraphicsItem* childItem;
	QColor color = QColorDialog::getColor(Qt::white, nullptr,
		QString("Choose fill color"), QColorDialog::ShowAlphaChannel);;
	foreach(childItem, childrenItems) {
		ComponentShape* childShape = dynamic_cast<ComponentShape*>(childItem);
		if (childShape && childShape->isSelected()) {
			childShape->changeBrushColor(color);
