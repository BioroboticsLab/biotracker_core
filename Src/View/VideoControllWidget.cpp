...
VideoControllWidget::VideoControllWidget(QWidget*     parent,
...
, ui(new Ui::VideoControllWidget)
{
    ...
    m_iconPause.addFile(QStringLiteral("qrc:/Images/resources/pause-sign.png"),
                        ...
                        QIcon::Off);
    m_iconPlay.addFile(
        QStringLiteral("qrc:/Images/resources/arrow-forward1.png"),
        QSize(),
        QIcon::Normal,
        QIcon::Off);
...
{
    ControllerPlayer* controller = dynamic_cast<ControllerPlayer*>(
        getController());
    controller->changeImageView(str);
}

void VideoControllWidget::setVideoViewComboboxModel(
...
void VideoControllWidget::getNotified()
{
    ...
    if (m_Paus) {
        ui->actionPlay_Pause->setIcon(
            QIcon(":/Images/resources/pause-sign.png"));
    } else {
        ui->actionPlay_Pause->setIcon(
            QIcon(":/Images/resources/arrow-forward1.png"));
    }

    m_RecI = mediaPlayer->getRecIState();
    if (m_RecI) {
        ui->actionRecord_cam->setIcon(
            QIcon(":/Images/resources/recordingCam.png"));
    } else {
        ui->actionRecord_cam->setIcon(
            QIcon(":/Images/resources/recordCam.png"));
    }

    int currentFrameNr = static_cast<int>(
        mediaPlayer->getCurrentFrameNumber());
    int totalNumberOfFrames = static_cast<int>(
        mediaPlayer->getTotalNumberOfFrames());
    ...
    QString currentVideoTime =
        QDateTime::fromMSecsSinceEpoch(
            ((float) currentFrameNr / (float) mediaFps) * 1000)
            .toUTC()
    ...
    if (totalNumberOfFrames >= 1) {
        ...
        int tickInterval  = pow(
            10,
            intervalPower > 0 ? floor(log10(totalNumberOfFrames)) - 1 : 0);
...
//     dynamic_cast<ControllerPlayer*>(getController());
...
//     ControllerPlayer* controller =
//     dynamic_cast<ControllerPlayer*>(getController()); controller->stop();
// }
...
/**
 * If the video slider is moved, this function sets the value to the current
 * frame number lable
 */
void VideoControllWidget::on_sld_video_sliderMoved(int position)
{
    ...
    MediaPlayer* mediaPlayer      = dynamic_cast<MediaPlayer*>(getModel());
    int          mediaFps         = mediaPlayer->getFpsOfSourceFile();
    QString      currentVideoTime = QDateTime::fromMSecsSinceEpoch(
                                   ((float) position / (float) mediaFps) *
                                   1000)
                                   .toUTC()
                                   .toString("hh:mm:ss:zzz");
    ui->time_edit->setText(currentVideoTime);
...
void VideoControllWidget::on_actionPlay_Pause_triggered(bool checked)
{
    ...
    if (m_Paus) {
        controller->pause();
        // qDebug() << "The average fps of this run was: " << _fpsSum /
        // _fpsCounter;
    } else {
        controller->play();
        _fpsSum     = 0;
...
void VideoControllWidget::on_actionScreenshot_triggered(bool checked)
{
    ...
    QPushButton* openFileButton    = msgBox.addButton(tr("Open screenshot"),
                                                   QMessageBox::ActionRole);
