...
void RectDescriptor::setRect(std::vector<cv::Point> rect)
{
    ...
    for (int i = 0; i < 4; i++) {
        std::shared_ptr<QGraphicsRectItem> ri =
            std::make_shared<QGraphicsRectItem>(
                QRect(rect[i].x - 10, rect[i].y - 10, 20, 20),
                this);
        ri->setBrush(_brush);
        rectification.push_back(ri);
        // Numbers at corners
        if ((dynamic_cast<AreaInfoElement*>(getModel()))->getShowNumbers()) {
            std::shared_ptr<QGraphicsSimpleTextItem> ti =
                std::make_shared<QGraphicsSimpleTextItem>(QString::number(i),
                                                          this);
    ...
    for (int i = 0; i < 4; i++) {
        ...
        std::shared_ptr<QGraphicsLineItem> ri =
            std::make_shared<QGraphicsLineItem>(QLine(fst->rect().x() + 10,
                                                      fst->rect().y() + 10,
                                                      snd->rect().x() + 10,
                                                      snd->rect().y() + 10),
                                                this);
        ...
        if ((dynamic_cast<AreaInfoElement*>(getModel()))->getShowNumbers()) {
            ...
            std::shared_ptr<QGraphicsSimpleTextItem> ti =
                std::make_shared<QGraphicsSimpleTextItem>(label.c_str(), this);
...
void RectDescriptor::receiveDragUpdate(BiotrackerTypes::AreaType vectorType,
                                       int                       id,
                                       double                    x,
                                       double                    y)
{
...
void RectDescriptor::paint(QPainter*                       painter,
                           ...
                           QWidget*                        widget)
{
    ...
    if (_dragVectorId >= 0 && _dragType != BiotrackerTypes::AreaType::NONE) {
        ...
        painter->drawLine(QLine(fst->rect().x() + 10,
                                fst->rect().y() + 10,
                                _drag.x(),
                                _drag.y()));
        painter->drawLine(QLine(snd->rect().x() + 10,
                                snd->rect().y() + 10,
                                _drag.x(),
                                _drag.y()));
    }
...
void RectDescriptor::updateLinePositions()
{
    ...
    for (int i = 0; i < 4; i++) {
        ...
        ln->setLine(QLine(fst->rect().x() + 10,
                          fst->rect().y() + 10,
                          snd->rect().x() + 10,
                          snd->rect().y() + 10));
    }
