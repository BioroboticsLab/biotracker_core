...
: AreaDescriptor(controller, model)
{
    ...
    for (int i = 0; i < 4; i++) {
        std::shared_ptr<QGraphicsRectItem> ri =
            std::make_shared<QGraphicsRectItem>(
                QRect(_v[i].x - 10, _v[i].y - 10, 20, 20),
                this);
        ri->setBrush(_brush);
    }
...
void EllipseDescriptor::updateEllipse()
{
    ...
    _v = (dynamic_cast<AreaInfoElement*>(getModel()))->getVertices();
    _rectification = std::make_shared<QGraphicsEllipseItem>(
        QRect(_v[0].x, _v[0].y, _v[1].x - _v[0].x, _v[1].y - _v[0].y),
        this);
    _rectificationMarkerOrig = std::make_shared<QGraphicsRectItem>(
        QRect(_v[0].x, _v[0].y, 10, 10),
        this);
    _rectificationMarkerEnd = std::make_shared<QGraphicsRectItem>(
        QRect(_v[1].x, _v[1].y, 10, 10),
        this);
    _rectificationMarkerOrig->setBrush(_brush);
    _rectificationMarkerEnd->setBrush(_brush);
}
...
void EllipseDescriptor::receiveDragUpdate(BiotrackerTypes::AreaType vectorType,
                                          int                       id,
                                          double                    x,
                                          double                    y)
{
...
void EllipseDescriptor::paint(QPainter*                       painter,
                              ...
                              QWidget*                        widget)
{
    ...
    if (_dragVectorId >= 0) {
        ...
        auto fst = _dragVectorId != 0 ? _rectificationMarkerOrig
                                      : _rectificationMarkerEnd;
        bool inv =
            isInverted(fst->rect().x(), fst->rect().y(), _drag.x(), _drag.y());
        ...
        if (inv) {
            ...
            QBrush bbb(b);
            p.setBrush(bbb);
            p.drawEllipse(QRect(fst->rect().x(),
                                fst->rect().y(),
                                _drag.x() - fst->rect().x(),
                                _drag.y() - fst->rect().y()));
            ...
            // This part is way simpler, as we only need to draw a somewhat
            // transparent & filled ellipse
            QColor b;
            ...
            painter->drawEllipse(QRect(fst->rect().x(),
                                       fst->rect().y(),
                                       _drag.x() - fst->rect().x(),
                                       _drag.y() - fst->rect().y()));
        }
