...
/**
 * This class inherits from the IViewTrackedComponent class and is therefore
 * part of the Composite Pattern. This view is visualized in the media panel of
 * the main window on top of the  loaded medium. It creates a component shape
 * for each trajectory. The component shape is the visualized current entity of
 * the trajectory. Each time the tracking plugins singals tracking done to the
 * core app, all component shapes are updated (updateshapes(framnumber))
 */
class TrackedComponentView : public IViewTrackedComponent
{
    Q_OBJECT
public:
    TrackedComponentView(QGraphicsItem* parent     = 0,
...
public slots:
    ...
    ////////////// contextmenu actions (must be parameterless because Qt
    /// actions require it)

    /// add a new trajectory --> view updates componentshapes and generates a
    /// new component shape for that trajectory
    void addTrajectory();
    ...
    /// broadcast the movment to all selected componentshapes so they are moved
    /// too
...
private:
    ...
    std::map<int, std::shared_ptr<QGraphicsRectItem>>
                   _rectification;          /**< id of the component */
    ...
    std::map<ENUMS::COREPERMISSIONS, bool>
        m_permissions; /**< list of permissions to set for new component shapes
                        */
...
protected:
    ...
    int m_currentFrameNumber =
        0; /**< current visualized framenumber of the medium */
};
