...
void GraphicsView::mousePressEvent(QMouseEvent* event)
{
    ...
    if (event->buttons() & Qt::MidButton) {
        ...
        const QPoint  imagePositionInt = QPoint(imagePosition.x(),
                                               imagePosition.y());
        event->ignore();
...
void GraphicsView::mouseMoveEvent(QMouseEvent* event)
{
    ...
    if (event->buttons() & Qt::MidButton) {
        ...
        if (currentPosition != m_ViewportDragOrigin) {
            ...
            QGraphicsView::horizontalScrollBar()->setValue(
                horizontalScrollBar()->value() + offset.x() + 1);
            QGraphicsView::verticalScrollBar()->setValue(
                verticalScrollBar()->value() + offset.y() + 1);
