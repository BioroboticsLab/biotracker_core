    ...
    for (int i = 1; i <= m_tracingLength && i <= m_currentFramenumber;
        ...
        if (historyChild && historyChild->getValid()) {
            ...
            if (m_tracerFrameNumber) {
                ...
                tracerNumberText->setPos(
                    adjustedHistoryPointDifference +
                    QPointF(-m_w * m_tracerProportions / 2.0f,
                            -m_h * m_tracerProportions / 5));
            }
        }
    }
}
...
void ComponentShape::setPermission(
...
void ComponentShape::setMembers(CoreParameter* coreParams)
{
    ...
    m_rotationHandle = new RotationHandle(QPoint(m_w / 2, m_h / 2),
                                          m_rotationHandleLayer);
    m_rotationHandle->setAntialiasing(m_antialiasing);
    QObject::connect(m_rotationHandle,
                     &RotationHandle::emitShapeRotation,
                     this,
                     &ComponentShape::receiveShapeRotation);
...
{
    QColor newColor = QColorDialog::getColor(m_penColor,
                                             nullptr,
...
void ComponentShape::createInfoWindow()
{
    ...
    infoList.append(
        QPair<QString, QString>("Framenumber",
                                QString::number(m_currentFramenumber)));
    infoList.append(
        QPair<QString, QString>("Seen for frames",
                                QString::number(m_trajectory->validCount())));
    ...
    infoList.append(QPair<QString, QString>("Orientation (in Degrees)",
                                            QString::number(m_rotation)));
    infoList.append(
        QPair<QString, QString>("Fixed", QString::number(m_fixed)));
    ...
    for (info = infoList.constBegin(); info != infoList.constEnd(); ++info) {
        ...
        infoKey->setFlags(Qt::NoItemFlags | Qt::ItemIsEnabled |
                          Qt::ItemIsSelectable);
        infoKeyInfo->setFlags(Qt::NoItemFlags | Qt::ItemIsEnabled |
                              Qt::ItemIsSelectable);
    ...
    const QString title      = QString("Information for track %1 on frame %2")
...
void ComponentShape::receiveShapeRotation(double angle, bool rotateEntity)
{
    ...
    if (rotateEntity) {
        ...
        Q_EMIT emitEntityRotation(m_trajectory,
                                  oldAngleNorm,
                                  toAngleNorm,
