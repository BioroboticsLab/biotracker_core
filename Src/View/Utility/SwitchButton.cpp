...
SwitchButton::SwitchButton(QString s0, QString s1, QWidget* parent)
...
void SwitchButton::paintEvent(QPaintEvent* e)
{
    ...
    if (isEnabled()) {
        ...
        p.drawRoundedRect(QRect(_margin,
                                _margin,
                                width() - 2 * _margin,
                                height() - 2 * _margin),
                          8.0,
        ...
        p.setBrush(_thumb);
        p.setOpacity(1.0);
        p.drawEllipse(QRectF(offset() - (_height / 2),
                             _y - (_height / 2),
                             height(),
                             height()));
        ...
        p.setFont(font);
        QRectF textRect = QRectF(_margin,
                                 _margin,
                                 width() - 2 * _margin,
                                 height() - 2 * _margin);
        p.drawText(textRect,
