...
static const std::string NUMBER_OF_PARTICLES      =
    "PARTICLEFILTERPARAM.NUMBER_OF_PARTICLES";
static const std::string GAUSSIAN_BLUR_SIZE       =
    "PARTICLEFILTERPARAM.GAUSSIAN_BLUR_SIZE";
static const std::string CLUSTERING_EPSILON       =
    "PARTICLEFILTERPARAM.CLUSTERING_EPSILON";
static const std::string MINIMUM_SCORE            =
    "PARTICLEFILTERPARAM.MINIMUM_SCORE";
static const std::string MAX_PARTICLES_PER_BUCKET =
    "PARTICLEFILTERPARAM.MAX_PARTICLES_PER_BUCKET";
static const std::string BUCKET_SIZE              =
    "PARTICLEFILTERPARAM.BUCKET_SIZE";
static const std::string PARTICLE_WIGGLE_DISTANCE =
    "PARTICLEFILTERPARAM.PARTICLE_WIGGLE_DISTANCE";
static const std::string NUM_CLUSTERS             =
...
class ParticleParams : public QObject {
  ...
  public:
    ...
    int      getClusteringEpsilon()      const {
        return _clusteringEpsilon;
    }
    double   getMinScore()               const {
        return _minScore;
    }
    unsigned getMaxParticlesPerBucket()  const {
        return _maxParticlesPerBucket;
    }
    unsigned getBucketSize()             const {
        return _bucketSize;
    }
    double   getParticleWiggleDistance() const {
        return _particleWiggleDistance;
    }
  ...
  private slots:
    ...
    void numPartChanged(int val)           {
        _numParticles = val;
    }
    void clusteringEpsChanged(int val)     {
        _clusteringEpsilon = val;
    }
    void minScoreChanged(double val)       {
        _minScore = val;
    }
    void maxBucketPartChanged(int val)     {
        _maxParticlesPerBucket = static_cast<unsigned>(val);
    }
    void bucketSizeChanged(int val)        {
        _bucketSize = static_cast<unsigned>(val);
    }
    void gaussianBlurSizeChanged(int val)  {
        _gaussianBlurSize = val;
    }
    void wiggleDistanceChanged(double val) {
        _particleWiggleDistance = val;
    }
    void numClustersChanged(int val)       {
        _numClusters = static_cast<unsigned>(val);
    }
