...
struct isYounger {
    bool operator()(const TrackedObject &lhs,
        ...
        return std::dynamic_pointer_cast<TrackedFish>
               (lhs.top())->age_of_last_known_position() <
...
void SimpleTracker::track(ulong frameNumber, const cv::Mat &frame) {
    ...
    for (TrackedObject &trackedObject : _trackedObjects) {
        ...
        float maxRange = std::min(age * MAX_TRACK_DISTANCE_PER_FRAME,
                                  MAX_TRACK_DISTANCE);
        ...
        for (const cv::Point2f &contour : contourCenters) {
            ...
            float distance = static_cast<float>(cv::sqrt((distPoint.x * distPoint.x) +
        ...
        if (minDistance < maxRange) {
            ...
            contourCenters.erase(std::find(contourCenters.begin(), contourCenters.end(),
                                           minDistanceContour));
        } else {
    ...
    if (_trackedObjects.size() < MAX_NUMBER_OF_TRACKED_OBJECTS) {
        ...
        for (FishCandidate &candidate : _fish_candidates) {
            ...
            for (const cv::Point2f &contour : contourCenters) {
                ...
                float distance = cv::sqrt((distPoint.x * distPoint.x) +
                                          (distPoint.y * distPoint.y));
                if (distance < minDistance) {
                    minDistance = distance;
                    minDistanceContour = contour;
            ...
            if (minDistance < std::min(MAX_TRACK_DISTANCE_PER_FRAME *
                                       candidate.age_of_last_known_position(), MAX_TRACK_DISTANCE)) {
        ...
        for (FishCandidate &promoted : candidates_to_promote) {
            ...
            newFish->set_associated_color(cv::Scalar(rng.uniform(0, 255), rng.uniform(0,
                                          255), rng.uniform(0, 255)));
            _fish_candidates.erase(std::find(_fish_candidates.begin(),
                                             _fish_candidates.end(), promoted));
...
void SimpleTracker::paint(ProxyPaintObject &p, const View &) {
    ...
    for (TrackedObject &trackedObject : _trackedObjects) {
        ...
        cv::circle(image, fish->last_known_position(), 3, fish->associated_color(), -1,
                   8, 0);
    }
