...
cv::Scalar BeesBookTagMatcher::COLOR_BLUE = cv::Scalar(255, 0, 0);
cv::Scalar BeesBookTagMatcher::COLOR_RED = cv::Scalar(0, 0, 255);
cv::Scalar BeesBookTagMatcher::COLOR_GREEN = cv::Scalar(0, 255);
cv::Scalar BeesBookTagMatcher::COLOR_YELLOW = cv::Scalar(0, 255, 255);


BeesBookTagMatcher::BeesBookTagMatcher(Settings & settings,
		std::string &serializationPathName, QWidget *parent) :
	...
	if (e->button() == Qt::LeftButton) {
		...
		//without modifier
			...
			if (_activeTag) // The Tag is active and can now be modified
			{
				...
				if (dist(g.centerGrid, cv::Point(e->x(), e->y())) > 2
						&& dist(g.centerGrid, cv::Point(e->x(), e->y()))
								< g.axesGrid.height)
...
void BeesBookTagMatcher::mouseReleaseEvent(QMouseEvent * e) {
	...
	if (e->button() == Qt::LeftButton) {
		if (_setTag) //center and orientation of the tag were set.
		{
			...
			_activeTag = true;
			_ready = true;
			g = myNewGrid(orient[0],
					atan2(orient[1].x - orient[0].x, orient[1].y - orient[0].y)
							- M_PI / 2);
...
void BeesBookTagMatcher::drawSetTags(cv::Mat image) {
	for (size_t i = 0; i < _Grids.size(); i++) {
		gtemp = myNewGrid(_Grids[i].scale, _Grids[i].centerGrid,
				_Grids[i].alpha, _Grids[i].theta, _Grids[i].phi, _Grids[i].ID);
		gtemp.drawFullTag(image, 2); //the grids are drawn as set
	}
}
//function that draws the orientation vector while being set.
void BeesBookTagMatcher::drawOrientation(cv::Mat image,
		std::vector<cv::Point> orient) {
...
void BeesBookTagMatcher::drawActiveTag(cv::Mat image) {
	...
	g.updatePoints();
	for (int i = 0; i < 3; i++) {
		cv::circle(image, g.absPoints[i], 1, this->COLOR_RED, 1); //the point is drawn in red
	}
	//active point in blue

	if (_setP0)
		cv::circle(image, g.absPoints[0], 1, this->COLOR_BLUE, 1);
	else if (_setP1)
		cv::circle(image, g.absPoints[1], 1, this->COLOR_BLUE, 1);
	else if (_setP2)
		cv::circle(image, g.absPoints[2], 1, this->COLOR_BLUE, 1);
...
void BeesBookTagMatcher::drawSettingTheta(cv::Mat &img) {
	...
	cv::ellipse(img, g.absPoints[0], g.axesTag, g.angleTag, 0, 360, this->COLOR_YELLOW, 1);
	cv::line(img, g.absPoints[0], g.absPoints[0] + g.realCoord[1], this->COLOR_RED, 1);	//the orientation vector is printed in red
	cv::line(img, g.absPoints[0], g.absPoints[0] + g.realCoord[2], this->COLOR_GREEN, 1);//the redius vector is printed in green

	for (int i = 0; i < 3; i++) {
		cv::circle(img, g.absPoints[0] + g.realCoord[i], 1, this->COLOR_RED, 1); //the point is drawn in red
	}
	if (_setP1)
		cv::circle(img, g.absPoints[0] + g.realCoord[1], 1, this->COLOR_BLUE, 1);
	else if (_setP2)
		cv::circle(img, g.absPoints[0] + g.realCoord[2], 1, this->COLOR_BLUE, 1);
...
void BeesBookTagMatcher::setTheta(cv::Point location) {
	...
	//when P2 is being modified
		...
		angle = atan2(location.x - g.centerGrid.x, location.y - g.centerGrid.y)
				- M_PI / 2;
...
void BeesBookTagMatcher::cancelTag(cv::Point2f location) {
	if (_Grids.size() > 0) {
		g = myNewGrid(_Grids[_Grids.size() - 1].scale,
				_Grids[_Grids.size() - 1].centerGrid,
				_Grids[_Grids.size() - 1].alpha,
				_Grids[_Grids.size() - 1].theta, _Grids[_Grids.size() - 1].phi,
				_Grids[_Grids.size() - 1].ID); //previous Tag is loaded
		_Grids.pop_back(); //last tag is set as active and removed from the vector
	} else
		//when there is just one tag, this is reset
...
void BeesBookTagMatcher::selectTag(cv::Point location) {
	if (_Grids.size() > 0)
		for (size_t i = 0; i < _Grids.size(); i++) {
			...
			if (dist(location, _Grids[i].centerGrid)
					< _Grids[i].axesTag.height) {
				_Grids.push_back(g); //active tag is pushed back in the vector of Grids
				g = myNewGrid(_Grids[i].scale, _Grids[i].centerGrid,
						_Grids[i].alpha, _Grids[i].theta, _Grids[i].phi,
						_Grids[i].ID); //selected Tag is loaded
