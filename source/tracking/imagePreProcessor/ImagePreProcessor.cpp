...
{	
	...
	if(image.data)
		cv::threshold(image, binarizedImage, _settings.getValueOfParam<int>(TRACKERPARAM::THRESHOLD_BINARIZING), 255, cv::THRESH_BINARY);
...
{	
	cv::Mat erodedImage;
	cv::Mat erodeKernel;
	if(_settings.getValueOfParam<int>(TRACKERPARAM::SIZE_ERODE) > 0)
	{
		erodeKernel = cv::getStructuringElement(cv::MORPH_CROSS, cv::Size(_settings.getValueOfParam<int>(TRACKERPARAM::SIZE_ERODE),_settings.getValueOfParam<int>(TRACKERPARAM::SIZE_ERODE)));
	} else {
		return image;
	}
	if(image.data)
...
{	
	cv::Mat dilatedImage;
	cv::Mat dilateKernel;
	if(_settings.getValueOfParam<int>(TRACKERPARAM::SIZE_DILATE) > 0)
	{
		dilateKernel = cv::getStructuringElement(
			cv::MORPH_RECT, 
			cv::Size(_settings.getValueOfParam<int>(TRACKERPARAM::SIZE_DILATE),
			_settings.getValueOfParam<int>(TRACKERPARAM::SIZE_DILATE)));
	} else {
		return image;
	}
	if(image.data)
...
cv::Mat ImagePreProcessor::backgroundSubtraction(cv::Mat image)
{
	...
	double alpha = _settings.getValueOfParam<double>(TRACKERPARAM::BG_MOG2_BACKGROUND_RATIO);
...
void ImagePreProcessor::preProcess(cv::Mat image)
{
	...
	if(_settings.getValueOfParam<bool>(TRACKERPARAM::ENABLE_BG_SUBTRACTION)) // 1. step: do the background subtraction
	{
	...
	if(_settings.getValueOfParam<bool>(TRACKERPARAM::ENABLE_BINARIZING)) // 2. step: binarize the image 
	{
		_binarizedImage = binarize(_proceededImage);
		_binarizedImage.copyTo(_proceededImage);
	}
	
	if(_settings.getValueOfParam<bool>(TRACKERPARAM::ENABLE_ERODE)) // 3. step: erode the image
	{
		_erodedImage = erode(_proceededImage);
		_erodedImage.copyTo(_proceededImage);
	}

	if(_settings.getValueOfParam<bool>(TRACKERPARAM::ENABLE_DILATE)) // 4. step: dilate the image
	{
...
{
	if(enableProcess.compare(TRACKERPARAM::ENABLE_BG_SUBTRACTION) == 0)
	{
		_backgroundSubtractionEnabled = enabled;
		return;
	}

	if(enableProcess.compare(TRACKERPARAM::ENABLE_BG_IMAGE) == 0)
	{
		_backgroundEnabled = enabled;
		return;
	}

	if(enableProcess.compare(TRACKERPARAM::ENABLE_BINARIZING) == 0)
	{
		_binarizeEnabled = enabled;
		return;
	}

	if(enableProcess.compare(TRACKERPARAM::ENABLE_ERODE) == 0)
	{
		_erodeEnabled = enabled;
		return;
	}

	if(enableProcess.compare(TRACKERPARAM::ENABLE_DILATE) == 0)
	{
...
cv::Mat ImagePreProcessor::imageDisplayType(TRACKERPARAM::IMAGE_DISPLAY_TYPE imageDisplayType)
{
	switch(imageDisplayType)
	{
		case TRACKERPARAM::IMAGE_DISPLAY_TYPE::IMAGE_BI:
			if(_settings.getValueOfParam<bool>(TRACKERPARAM::ENABLE_BINARIZING))
				return _binarizedImage;
			break;
		case TRACKERPARAM::IMAGE_DISPLAY_TYPE::IMAGE_ER:
			if(_settings.getValueOfParam<bool>(TRACKERPARAM::ENABLE_ERODE))
				return _erodedImage;
			break;
		case TRACKERPARAM::IMAGE_DISPLAY_TYPE::IMAGE_DI:
			if(_settings.getValueOfParam<bool>(TRACKERPARAM::ENABLE_DILATE))
				return _dilatedImage;
			break;
		case TRACKERPARAM::IMAGE_DISPLAY_TYPE::IMAGE_FG:
			if(_settings.getValueOfParam<bool>(TRACKERPARAM::ENABLE_BG_SUBTRACTION))
				return _fgMaskMOG;
			break;
		case TRACKERPARAM::IMAGE_DISPLAY_TYPE::IMAGE_BG:
			if(_settings.getValueOfParam<bool>(TRACKERPARAM::ENABLE_BG_SUBTRACTION) && _backgroundEnabled)
				return _backgroundImage;
			break;
		case TRACKERPARAM::IMAGE_DISPLAY_TYPE::IMAGE_PP:
