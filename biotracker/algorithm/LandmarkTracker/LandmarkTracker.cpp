...
#include "biotracker/tracking/TrackingThread.h"
...

{
    ...
    QObject::connect(this->_roiButton, SIGNAL(clicked()), this,
                     SLOT(setLabelRoi()));
    QObject::connect(this->_sampledRoiButton, SIGNAL(clicked()), this,
                     SLOT(setLabelSampledRoi()));

    if (_trackedObjects.empty()) {
        const size_t dummyID = 0;
        _trackedObjects.emplace_back(dummyID);
    } else if (_trackedObjects.size()>1) {
        std::cerr << "Unexpected number of trackedObjects: " << _trackedObjects.size()
                  << std::endl;
    }
}


void LandmarkTracker::track(ulong /* frameNumber */,
                            const cv::Mat & /* frame */) {
    setRect();
}
...
void LandmarkTracker::drawRectangle(cv::Mat image) {
    ...
    if (_lockRect) {
        cv::putText(image, rectInfo(), pointOfText(), cv::FONT_HERSHEY_PLAIN, 0.8,
                    COLOR_RECTANGLE, 1, 8, false);
    }
}
...
void LandmarkTracker::addRect2TrackedObj() {
    ...
    _rectObject = std::make_shared<RectObject>(_frameNr, _selectorRecStart,
                  _selectorRecEnd);
...
QPixmap LandmarkTracker::mat2QPixmap(const cv::Mat &mat) {
    ...
    QImage dest(static_cast<const unsigned char *>(rgb.data), rgb.cols, rgb.rows,
...
void LandmarkTracker::mouseMoveEvent(QMouseEvent *e) {
    ...
    const bool _leftMouseHold = QApplication::mouseButtons().testFlag(
                                    Qt::LeftButton);
    const bool _rightMouseHold = QApplication::mouseButtons().testFlag(
                                     Qt::RightButton);
    ...
    if (_rightMouseHold /*&& ctrlModifier*/ && !_lockRect) {
        ...
        if (cv::Rect(_selectorRecStart, _selectorRecEnd).contains(cv::Point(e->x(),
                e->y()))) {
            _mouseStop.x = e->x();
            _mouseStop.y = e->y();
...
void LandmarkTracker::mouseReleaseEvent(QMouseEvent *e) {
    if (e->button() == Qt::LeftButton || e->button() == Qt::RightButton) {
        ...
        std::string note = "selected area from " + QString::number(
                               _selectorRecStart.x).toStdString() + ":"+ QString::number(
                               _selectorRecStart.y).toStdString()
...
void LandmarkTracker::initParamsWidget() {
    ...
    sizePolicy.setHeightForWidth(
        _sampledRoiInParams->sizePolicy().hasHeightForWidth());
    ...
    sizePolicy2.setHeightForWidth(
        _sampledRoiButton->sizePolicy().hasHeightForWidth());
...
void LandmarkTracker::updateParamsWidget() {
    ...
    _roiInParams->setPixmap(_pixmap.scaled(_roiFrame->size(), Qt::KeepAspectRatio,
                                           Qt::FastTransformation));
    _roiInParams->show();

    _pixmap = mat2QPixmap(sampledRoi);
    _sampledRoiInParams->setPixmap(_pixmap.scaled(_roiFrame->size(),
...
void LandmarkTracker::forcePointIntoBorders(cv::Point &point,
        cv::Rect const &borders) {
