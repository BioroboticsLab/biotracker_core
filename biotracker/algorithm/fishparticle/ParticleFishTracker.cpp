...
void ParticleFishTracker::track(unsigned long, const cv::Mat &frame) {
    ...
    try {
        ...
        if (_current_particles.empty()) {
            ...
            std::sort(_current_particles.begin(), _current_particles.end(),
                      compareReverseParticleScorePredicate());
        ...
        std::copy_if(_current_particles.begin(), _current_particles.end(),
                     std::back_inserter(particles_high_scores),
        [score_cutoff](const Particle& p) {
            return p.getScore() >= score_cutoff;
        });
...
void ParticleFishTracker::importanceResample() {
    GridParticleBuckets buckets(_params.getMaxParticlesPerBucket(),
                                _prepared_frame.rows, _prepared_frame.cols, _params.getBucketSize(),
                                _params.getBucketSize());
    ...
    seedParticles(random_new_particles, 0, 0, _prepared_frame.cols,
                  _prepared_frame.rows);
}
...
void ParticleFishTracker::wiggleParticle(Particle &to_wiggle) {
    ...
    to_wiggle.setX(to_wiggle.getX() + static_cast<float>(_rng.gaussian(
                       wiggle_distance)));
    to_wiggle.setY(to_wiggle.getY() + static_cast<float>(_rng.gaussian(
                       wiggle_distance)));
...
*/
void ParticleFishTracker::seedParticles(size_t num_particles, int min_x,
...
void ParticleFishTracker::paintOverlay(QPainter *painter) {
    ...
    auto drawParticle = [&](const Particle &particle, const QColor &color) {
        ...
        pen.setColor(color);
        painter->setPen(pen);
        painter->drawEllipse(QPoint(static_cast<int>(x), static_cast<int>(y)), size,
                             size);
    };
    ...
    for (const Particle &particle : _current_particles) {
        if (_min_score >= _max_score) {
            ...
            unsigned scaled_score = static_cast<unsigned>((particle.getScore() -
                                    _min_score) / (_max_score - _min_score) * 220.f);
